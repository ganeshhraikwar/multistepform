    ...
    <div className="w-full h-full md:w-[274px] md:h-[568px] flex-shrink-0 bg-purplish-blue md:rounded-2xl p-8 pt-10 flex flex-row md:flex-col justify-center md:justify-start items-start md:gap-8 relative overflow-hidden space-x-4 md:space-x-0">
      ...
      <svg className="absolute bottom-0 left-0 w-full md:h-auto object-cover pointer-events-none md:bottom-0 z-0 h-full md:h-[auto] block md:hidden" preserveAspectRatio="none" viewBox="0 0 375 172" fill="none" xmlns="http://www.w3.org/2000/svg">
        <motion.path 
          ...
          animate={{ y: [0, -5, 0], scaleY: [1, 1.05, 1] }} transition={{ duration: 3, repeat: Infinity, ease: "easeInOut" }}
        />
        <motion.path 
          ...
          animate={{ y: [0, 5, 0], x: [0, -5, 0] }} transition={{ duration: 4, repeat: Infinity, ease: "easeInOut", delay: 1 }}
        />
        <motion.path 
          d="M0 172H210C210 172 185 135.5 115 134C45 132.5 17 116.5 0 116.5V172Z" fill="#6259FF" 
          animate={{ y: [0, -4, 0], scaleY: [1, 1.03, 1] }} transition={{ duration: 5, repeat: Infinity, ease: "easeInOut", delay: 2 }}
        />
      </svg>
      <svg className="absolute bottom-0 left-0 w-full h-auto object-cover pointer-events-none hidden md:block" viewBox="0 0 274 274" fill="none" xmlns="http://www.w3.org/2000/svg">
        <motion.path fillRule="evenodd" clipRule="evenodd" d="M0 274H274V112.5C274 112.5 240 92 205 130C170 168 139 125 110 150C81 175 42 162 0 174V274Z" fill="#F9818E" animate={{ y: [0, -8, 0], scaleY: [1, 1.02, 1] }} transition={{ duration: 8, repeat: Infinity, ease: "easeInOut" }} />
        <motion.path fillRule="evenodd" clipRule="evenodd" d="M0 274H274V154.5C274 154.5 220.5 139.5 190.5 186.5C160.5 233.5 125 214.5 97.5 236C70 257.5 35 249.5 0 263.5V274Z" fill="#FFAF7E" animate={{ y: [0, 6, 0], x: [0, -4, 0] }} transition={{ duration: 10, repeat: Infinity, ease: "easeInOut", delay: 1 }} />
        <motion.path d="M0 274H154C154 274 135.5 217 84.5 214.5C33.5 212 12.5 187 0 187V274Z" fill="#6259FF" animate={{ y: [0, -4, 0] }} transition={{ duration: 12, repeat: Infinity, ease: "easeInOut", delay: 2 }} />
        <motion.path d="M0 274H74C74 274 54 227 0 224V274Z" fill="#473DFF" animate={{ y: [0, 5, 0] }} transition={{ duration: 9, repeat: Infinity, ease: "easeInOut", delay: 1.5 }} />
        <motion.circle cx="107" cy="188" r="3" fill="white" animate={{ opacity: [0.2, 1, 0.2] }} transition={{ duration: 1.5, repeat: Infinity }} />
        <motion.circle cx="127" cy="198" r="3" fill="white" animate={{ opacity: [0.2, 1, 0.2] }} transition={{ duration: 1.5, repeat: Infinity, delay: 1 }} />
